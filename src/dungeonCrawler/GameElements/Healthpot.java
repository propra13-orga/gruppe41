...
public class Healthpot extends GameElement {
	 ...
	 * @param size
	 */
	public Healthpot(Vector2d position, Vector2d size, int id) {
	 ...
	 * @return a {@link Healthpot} instance
	 */
	public static Healthpot createElement(String[] param, int id) {
			if (param.length > 5) {
				element = new Healthpot(new Vector2d(), new Vector2d(), Integer.parseInt(param[1]));
			}
			else {
				element = new Healthpot(new Vector2d(), new Vector2d(), id);
			}
		modify(param);
		return element;
	}
