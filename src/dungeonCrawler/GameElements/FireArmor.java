 ...
 *
 */
public class FireArmor extends GameElement {
	static FireArmor element;
	 ...
	 */
	@Deprecated
	public FireArmor(Vector2d position, Vector2d size) {
	 ...
	 * @param size
	 */
	public FireArmor(Vector2d position, Vector2d size, int id) {
	...
	public void GameEventPerformed(GameEvent e) {
		...
		if(e.element instanceof Player && e.type == EventType.COLLISION){
			System.out.println("Feuerrüstung aufgenommen");
			Player elementPlayer = (Player) e.element;
			elementPlayer.addItem(new dungeonCrawler.GameObjects.FireArmor(5));
			this.size.setX(0);
			this.size.setY(0);
		}
	}
	 ...
	 * @return a {@link FireArmor} instance
	 */
	public static FireArmor createElement(String[] param, int id) {
			if (param.length > 5) {
				element = new FireArmor(new Vector2d(), new Vector2d(), Integer.parseInt(param[1]));
			}
			else {
				element = new FireArmor(new Vector2d(), new Vector2d(), id);
			}
	...
	public void modify(String[] param) {
		...
		try {
			...
			System.out.println("Kann FIREARMOR-Parameter nicht interpretieren.");
			element = null;
		}
	}
